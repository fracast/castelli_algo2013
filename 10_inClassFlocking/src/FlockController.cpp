...
//            b>>2;
            ...
            if( lenSq < zoneRadiusSq ){
                ...
                // align! alignement directionally and also magnitude
                // same speed and direction and you have an alignment
                else if( pct < alignThresh ){
                    
                    float alignStrength = 0.005;
                    float adjustedPct = ofMap(pct, separateThresh, alignThresh, 0.0, 1.0);
                    float F = adjustedPct * alignStrength;
                ...
                // attract!
                    ...
                    ofVec3f force = dir.normalized() * adjustedPct * attractStrength;
                    b1->applyForce( -force );
                    b2->applyForce( force );
                }
