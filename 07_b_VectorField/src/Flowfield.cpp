...
//NEW WAY NUMBER 1
void FlowField::sinYsinX(){
    for( int y=0; y<flowList.size(); y++){
        for( int x=0; x<flowList[y].size(); x++){
            flowList[y][x].set(ofVec2f(PI * (x),PI * (y)));
        }
    }
}

//NEW WAY NUMBER 2
void FlowField::negYposX(float radius, float strength){
    for( int y=0; y<flowList.size(); y++){
        for( int x=0; x<flowList[y].size(); x++){
            ofVec2f np(y*resolution*PI, x*resolution*PI);
            
            
            if( np.distance(pos) < radius ){
                float pct = 1 - (np.distance(pos) / radius);
...
//NEW WAY NUMBER 3
void FlowField::sinXcosY(){
    for( int y=0; y<flowList.size(); y++){
        for( int x=0; x<flowList[y].size(); x++){
            flowList[y][x].set(ofVec2f(sin(x),cos(y)));
        }
    }
}
